...
class BytesIsotpUplinkConverter(IsotpConverter):
    @StatisticsService.CollectStatistics(start_stat_type='receivedBytesFromDevices',
                                         end_stat_type='convertedBytesFromDevice')
    def convert(self, configs, isotp_data):
        result = {"attributes": {},
                  "telemetry": {}}
        ...
        for config in configs:
            try:
                ...
                data_length = config["length"] if config["length"] != -1 else len(isotp_data) - config["start"]

                # The 'value' variable is used in eval
                if config["type"][0] == "b":
                    value = bool(isotp_data[config["start"]])
                elif config["type"][0] == "i" or config["type"][0] == "l":
                    value = int.from_bytes(isotp_data[config["start"]:config["start"] + data_length],
                                           config["byteorder"],
                    ...
                    value = struct.unpack_from(fmt,
                                               bytes(isotp_data[config["start"]:config["start"] + data_length]))[0]
                elif config["type"][0] == "s":
                    value = isotp_data[config["start"]:config["start"] + data_length].decode(config["encoding"])
                elif config["type"][0] == "r":
                    value = ""
                    for hex_byte in isotp_data[config["start"]:config["start"] + data_length]:
                        value += "%02x" % hex_byte
                else:
                    log.error("Failed to convert ISO-TP data to TB %s '%s': unknown data type '%s'",
                ...
                if config.get("expression", ""):
                    result[tb_item][tb_key] = eval(config["expression"],
                                                   ...
                                                   {"value": value, "isotp_data": isotp_data})
                else:
                    result[tb_item][tb_key] = value
            except Exception as e:
                log.error("Failed to convert ISO-TP data to TB %s '%s': %s",
