...
from thingsboard_gateway.connectors.modbus.entities.bytes_uplink_converter_config import (
...
class Slave(Thread):
    def __init__(self, connector: "ModbusConnector", logger, config):
        ...
        if self.type == SERIAL_CONNECTION_TYPE_PARAMETER:
            self.name = (
                "Modbus slave processor for unit "
                + str(config[UNIT_ID_PARAMETER])
                + " on port "
                + str(config[PORT_PARAMETER])
                + " "
                + config[DEVICE_NAME_PARAMETER]
            )
        else:
            self.name = (
                "Modbus slave processor for unit "
        ...
        self.rs485 = config.get("rs485", False)
        self.rtscts = config.get("rtscts", False)
        self.dsrdtr = config.get("dsrdtr", False)
        self.xonxoff = config.get("xonxoff", False)
        ...
        self.connect_attempt_time_ms = (
            config.get(CONNECT_ATTEMPT_TIME_MS_PARAMETER, 500)
            if config.get(CONNECT_ATTEMPT_TIME_MS_PARAMETER, 500) > 500
            else 500
        )
        self.wait_after_failed_attempts_ms = (
            config.get(WAIT_AFTER_FAILED_ATTEMPTS_MS_PARAMETER, 30000)
            if config.get(WAIT_AFTER_FAILED_ATTEMPTS_MS_PARAMETER, 30000) > 30000
            else 30000
        )
        self.connection_attempt = (
            config.get(CONNECT_ATTEMPT_COUNT_PARAMETER, 2)
            if config.get(CONNECT_ATTEMPT_COUNT_PARAMETER, 2) >= 2
            else 2
        )
    ...
    def __load_converter(
        self, config, converter_type, converter_config: Union[Dict, BytesUplinkConverterConfig] = {}
    ):
        try:
            if isinstance(config.get(converter_type + CONVERTER_PARAMETER), str):
                converter = TBModuleLoader.import_module(
                    self.connector.connector_type, config[converter_type + CONVERTER_PARAMETER]
                ...
                if converter_type == DOWNLINK_PREFIX:
                    converter = BytesModbusDownlinkConverter(
                        converter_config, self.connector.converter_log
                    )
                else:
            ...
            self._log.exception(
                "Failed to load %s converter for %s slave: %s", converter_type, self.name, e
            )
    ...
    async def connect(self) -> bool:
        ...
        if not self.master.connected():
            if (
                self.connection_attempt_count >= self.connection_attempt
                and cur_time - self.last_connection_attempt_time
                >= self.wait_after_failed_attempts_ms
            ):
                self.connection_attempt_count = 0

            while (
                not self.master.connected()
                and self.connection_attempt_count < self.connection_attempt
                and (
                    cur_time - self.last_connection_attempt_time >= self.connect_attempt_time_ms
    ...
    async def read(self, function_code, address, objects_count):
        self._log.debug(
            "Reading %s registers from address %s with function code %s",
            objects_count,
            address,
            function_code,
        )
        ...
        StatisticsService.count_connector_message(
            self.connector.get_name(), stat_parameter_name="connectorMsgsReceived"
        )
    ...
    async def __read(self, function_code, address, objects_count):
        ...
        try:
            time_to_sleep = self.__master.get_time_to_pass_delay_between_requests(
                self.__delay_between_requests_ms
            )
            if time_to_sleep > 0:
                await asyncio.sleep(time_to_sleep / 1000)
            result = await self.available_functions[function_code](
                address=address, count=objects_count, unit_id=self.unit_id
            )
        except KeyError:
    ...
    async def write(self, function_code, address, value):
        self._log.debug(
            "Write %s value to address %s with function code %s", value, address, function_code
        )
    ...
    async def __write(self, function_code, address, value):
        ...
        try:
            ...
            if function_code in (5, 6):
                result = await self.available_functions[function_code](
                    address=address, value=value, unit_id=self.unit_id
                )
                ...
                result = await self.available_functions[function_code](
                    address=address, values=value, unit_id=self.unit_id
                )
            else:
                self._log.error("Unknown Modbus function with code: %s", function_code)
        except Exception as e:
