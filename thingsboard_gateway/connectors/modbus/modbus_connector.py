...
from thingsboard_gateway.gateway.constants import (
    ...
    DATA_PARAMETER,
    ATTRIBUTE_UPDATE_METHOD_PARAMETER,
    ATTRIBUTE_UPDATE_PARAMS_PARAMETER,
    ON_ATTRIBUTE_UPDATE_DEFAULT_TIMEOUT,
)
...
if installation_required:
    ...
    TBUtility.install_package("pymodbus", required_version)
...
from thingsboard_gateway.connectors.modbus.backward_compatibility_adapter import (
...
class AsyncModbusConnector(Connector, Thread):
    def __init__(self, gateway, config, connector_type):
        ...
        self.name = config.get(
            "name", "Modbus Connector " + "".join(choice(ascii_lowercase) for _ in range(5))
        )
        ...
        self.converter_log = init_logger(
            self.__gateway,
        ...
        self.__backward_compatibility_adapter = BackwardCompatibilityAdapter(
            config, gateway.get_config_path(), logger=self.__log
        )
    ...
    async def __run(self):
        ...
        await asyncio.gather(self.__process_requests(), self.__convert_data(), self.__save_data())
        ...
        if slave.uplink_converter_config.attributes or slave.uplink_converter_config.telemetry:
            try:
                ...
                if connected_to_master:
                    ...
                    self.__log.error(
                        "Device %s is not connected, cannot connect to server, skipping reading...",
                        slave,
                    )
    ...
    async def __read_slave_data(self, slave: Slave):
        ...
        for config_section in ("attributes", "telemetry"):
            for config in getattr(slave.uplink_converter_config, config_section):
                try:
                    response = await slave.read(
                        config["functionCode"], config["address"], config["objectsCount"]
                    )
                except asyncio.exceptions.TimeoutError:
                    self.__log.error(
                        "Timeout error for device %s function code %s address %s, it may be caused by wrong data in server register.",  # noqa
                        slave.device_name,
                        config["functionCode"],
                        config[ADDRESS_PARAMETER],
                    )
                    continue
    ...
    def __add_device_to_platform(self, slave: Slave):
        ...
        if slave.master.connected():
            device_connected = self.__gateway.add_device(
                slave.device_name, {CONNECTOR_PARAMETER: self}, device_type=slave.device_type
            )
    ...
    def on_attributes_update(self, content):
        ...
        try:
            ...
            if device is None:
                self.__log.error(
                    "Device %s not found in connector %s",
            ...
            if not attribute_update_config_list:
                self.__log.error(
                    "No attributes found that match attributes section for device %s", device.name
                )
                return

            self.__process_task_on_filtered_attribute_update_section(
                device, content, attribute_update_config_list
            )
        except Exception as e:
            self.__log.exception("Failed to update attributes: %s", e)
            self.__log.debug("Got exception: %s", e, exc_info=True)

    def __process_task_on_filtered_attribute_update_section(
        self, device: Slave, content: dict, attribute_update_config: list[dict]
    ):
        for attribute_update_config in attribute_update_config:
            try:
                ...
                attribute_update_params_parameter = content[DATA_PARAMETER][
                    attribute_update_method_parameter
                ]
                to_process = {
                ...
                task = self.__create_task(
                    self.__process_attribute_update,
                    (device, to_process, attribute_update_config),
                    {},
                )
                task_completed, result = self.__wait_task_with_timeout(
                    task=task, timeout=ON_ATTRIBUTE_UPDATE_DEFAULT_TIMEOUT, poll_interval=0.2
                )
                if not task_completed:
                    self.__log.error(
                        "Attribute update processing is timed out for attribute: %s",
                        attribute_update_config[TAG_PARAMETER],
                    )
                    continue
                self.__log.debug(
                    "Attribute update processed successfully for attribute: %s",
                    attribute_update_config[TAG_PARAMETER],
                )

            except KeyError as e:
                self.__log.error(
                    "Failed to extract necessary components for attribute: %s due to %s",
                    attribute_update_config[TAG_PARAMETER],
                    str(e),
                )
                continue
    ...
    def __wait_task_with_timeout(
        self, task: asyncio.Task, timeout: float, poll_interval: float = 0.2
    ) -> Tuple[bool, Any]:
        start_time = monotonic()
    ...
    async def __process_attribute_update(self, device: Slave, data, config):
        if config is not None:
            ...
            config = BytesDownlinkConverterConfig(
                ...
                objects_count=config.get(
                    "objectsCount", config.get("registersCount", config.get("registerCount", 1))
                ),
        ...
        if rpc_request.for_existing_device():
            for device in self.__slaves:
                ...
                try:
                    ...
                    task_completed, result = self.__wait_task_with_timeout(
                        task=task, timeout=rpc_request.timeout, poll_interval=0.2
                    )
                    if not task_completed:
                        self.__log.error(
                    ...
                    self.__log.debug(
                        "RPC with method %s execution result is: %s", rpc_request.method, result
                    )

                except Exception as e:
                    self.__log.exception("Failed to process server side rpc request: %s", e)
                    self.__gateway.send_rpc_reply(
                        rpc_request.device_name, rpc_request.id, {"result": {"error": str(e)}}
                    )
            return results
    ...
    def __process_connector_rpc_with_no_device(self, rpc_request: RPCRequest):
        try:
            ...
            while (
                not connected_to_master_task.done()
                and not self.__stopped
                and monotonic() - connect_started < rpc_request.timeout
            ):
                sleep(0.02)
        ...
        if connected_to_master_task.result():
            try:
                ...
                if not task_completed:
                    self.__log.error(
                        "Failed to process rpc request for device %s ,timeout has been reached ",
                        slave.device_name,
                    )
    ...
    def __process_reserved_rpc_request(self, rpc_request: RPCRequest):
        ...
        if device is None:
            self.__log.error(
                "Device %s not found in connector %s", rpc_request.device_name, self.get_name()
            )
            self.__gateway.send_rpc_reply(
        ...
        try:
            task = self.__create_task(
                self.__process_rpc_request, (device, rpc_request.params, rpc_request), {}
            )
            task_completed, result = self.__wait_task_with_timeout(
                task=task, timeout=rpc_request.timeout, poll_interval=0.2
            )
            ...
            if not task_completed:
                self.__log.error(
                    "Failed to process reserved rpc request for device %s ,timeout has been reached",
                    device.device_name,
                )
    ...
    def __process_device_rpc_request(self, rpc_request: RPCRequest):
        ...
        try:
            task = self.__create_task(
                self.__process_rpc_request, (device, device_rpc_config, rpc_request), {}
            )
            task_completed, result = self.__wait_task_with_timeout(
                task=task, timeout=rpc_request.timeout, poll_interval=0.2
            )
    ...
    async def __read_rpc_data(self, device: Slave, config):
        ...
        if isinstance(response, ModbusPDU):
            ...
            response = device.uplink_converter.decode_data(
                response, config, endian_order, word_endian_order
            )
    ...
    async def __write_rpc_data(self, device, config, data):
        ...
        if converted_data is not None:
            try:
                connected = await device.connect()
                if connected:
                    response = await device.write(
                        config.function_code, config.address, converted_data
                    )
            except Exception as e:
