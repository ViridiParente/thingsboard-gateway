...
from thingsboard_gateway.gateway.report_strategy.report_strategy_service import (
    ReportStrategyService,
)
from thingsboard_gateway.gateway.shell.proxy import AutoProxy
from thingsboard_gateway.gateway.statistics.decorators import (
    CountMessage,
    CollectStorageEventsStatistics,
    CollectAllSentTBBytesStatistics,
    CollectRPCReplyStatistics,
)
...
class TBGatewayService:
    ...
    def __init__(self, config_file=None):
        ...
        self._event_storage = self._event_storage_types[self.__config["storage"]["type"]](
            self.__config["storage"], storage_log, self.stop_event
        )
        if self.__config["thingsboard"].get("reportStrategy", {}).get("type") != "DISABLED":
            self._report_strategy_service = ReportStrategyService(
                self.__config["thingsboard"], self, self.__converted_data_queue, log
            )
        else:
        ...
        self.__sync_devices_shared_attributes_on_connect = self.__config["thingsboard"].get(
            ...
            self.send_telemetry(
                {
                    "ts": time() * 1000,
                    "values": {
                        "LOGS": "Logging loading exception, logs.json is wrong: %s"
                        % (str(logging_error),)
                    },
                }
            )
        ...
        self.__save_converted_data_thread = Thread(
            name="Storage fill thread", daemon=True, target=self.__send_to_storage
        )
        ...
        self.__rpc_processing_thread = Thread(
            target=self.__send_rpc_reply_processing, daemon=True, name="RPC processing thread"
        )
        self.__rpc_processing_thread.start()
        self.__rpc_to_devices_processing_thread = Thread(
            target=self.__rpc_to_devices_processing,
            daemon=True,
            name="RPC to devices processing thread",
        )
        self.__rpc_to_devices_processing_thread.start()

        self.__process_sync_device_shared_attrs_thread = Thread(
            target=self.__sync_device_shared_attrs_loop,
            daemon=True,
        ...
        self.init_statistics_service(
            self.__config["thingsboard"].get("statistics", DEFAULT_STATISTIC)
        )
        ...
        self.__max_payload_size_in_bytes = self.__config["thingsboard"].get(
        ...
        self.init_device_filtering(
            self.__config["thingsboard"].get("deviceFiltering", DEFAULT_DEVICE_FILTER)
        )
    ...
    def __init_variables(self):
        ...
        self.__messages_confirmation_executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=4
        )  # noqa
    ...
    def __load_general_config(config_file):
        ...
        if file_extension == "json" and os.path.exists(config_file):
            ...
            log.warning(
                "YAML configuration is deprecated. " "Please, use JSON configuration instead."
            )
            log.warning(
    ...
    def init_statistics_service(self, config):
        ...
        if self.__statistics.get("enable", False) or self.__statistics.get("enableCustom", False):
            ...
            self.__statistics_service = StatisticsService(
                ...
                log,
                config_path=(
                    self._config_dir + self.__statistics["configuration"]
        ...
        try:
            ...
            while not self.stopped:
                try:
                    ...
                    if (
                        ...
                        for device in list(self.__saved_devices.keys()):
                            self.add_device(
                                device,
                                {
                                    CONNECTOR_PARAMETER: self.__saved_devices[device][
                                        CONNECTOR_PARAMETER
                                    ]
                                },
                        ...
                        for rpc_call_index in range(len(self.__scheduled_rpc_calls)):
                            ...
                            if cur_time > rpc_call[0]:
                                ...
                                try:
                                    ...
                                    log.error(
                                        "Error while executing scheduled RPC call: %s", exc_info=e
                                    )
                    ...
                    if (
                        self.__rpc_requests_in_progress or not self.__rpc_register_queue.empty()
                    ...
                    if (
                        not self.__requested_config_after_connect
                        and self.tb_client.is_connected()
            ...
            try:
                ...
                while not self.available_connectors_by_id[current_connector].is_stopped():
                    ...
                    if self.tb_client.is_connected():
                        for device in self.get_connector_devices(
                            self.available_connectors_by_id[current_connector]
                        ):
    ...
    def __stop_gateway(self):
        ...
        if (
            hasattr(self, "_TBGatewayService__statistics_service")
            and self.__statistics_service is not None
        ):
            self.__statistics_service.stop()
    ...
    def __process_remote_logging_update(self, remote_logging_level):
        if remote_logging_level is not None:
            remote_logging_level_id = TBRemoteLoggerHandler.get_logger_level_id(
                remote_logging_level.upper()
            )
    ...
    def __process_remove_provisioned_credentials(self):
        ...
        if os.path.exists(self._config_dir + PROVISIONED_CREDENTIALS_FILENAME):
            try:
                ...
                execution_result = {
                    "code": 200,
                    "resp": "Provisioned credentials file removed successfully.",
                }
            except OSError as e:
            ...
            execution_result = {
                "code": 404,
                "message": "Provisioned credentials file does not exist.",
            }
    ...
    def _check_shared_attributes(self, shared_keys=None, client_keys=None):
        ...
        self.tb_client.client.request_attributes(
            callback=self._attributes_parse, shared_keys=shared_keys, client_keys=client_keys
        )
    ...
    def __register_connector(self, session_id, connector_key):
        if (
            ...
            log.info(
                "[%r][%r] GRPC connector with key %s registered with name %s",
                session_id,
                connector.get_id(),
                connector_key,
                connector.get_name(),
            ...
            log.error(
                "[%r] GRPC connector with key: %s - already registered!", session_id, connector_key
            )
        else:
            self.__grpc_manager.registration_finished(Status.NOT_FOUND, session_id, None)
            log.error(
                "[%r] GRPC configuration for connector with key: %s - not found",
                session_id,
                connector_key,
            )
    ...
    def __unregister_connector(self, session_id, connector_key):
        if (
            ...
            log.info(
                "[%r] GRPC connector with key %s and name %s - unregistered",
                session_id,
                connector_key,
            ...
            log.error(
                "[%r] GRPC connector with key: %s - is not registered!", session_id, connector_key
            )
        else:
            self.__grpc_manager.unregister(Status.FAILURE, session_id, None)
            log.error(
                "[%r] GRPC configuration for connector with key: %s - not found and not registered",
                session_id,
                connector_key,
            )
        ...
        if connectors_configuration_in_main_config:
            for connector_config_from_main in connectors_configuration_in_main_config:
                try:
                    ...
                    connector_type = (
                        connector_config_from_main["type"].lower()
                        if connector_config_from_main.get("type") is not None
                        else None
                    )
                    ...
                    if connector_type != "grpc":
                        ...
                        if (
                            ...
                            connector_persistent_key = self._generate_persistent_key(
                                connector_config_from_main, connectors_persistent_keys
                            )
                        else:
                            connector_class = TBModuleLoader.import_module(
                                connector_type,
                                self._default_connectors.get(
                                    connector_type, connector_config_from_main.get("class")
                                ),
                            )
                        ...
                        if connector_class is not None and isinstance(connector_class, list):
                            ...
                            for error in connector_class:
                                log.error(
                                    "The following error occurred during importing connector class: %s",
                                    error,
                                    exc_info=error,
                                )
                            continue
                    ...
                    connector_config_file_path = (
                        self._config_dir + connector_config_from_main["configuration"]
                    )

                    if not path.exists(connector_config_file_path):
                        log.error(
                            "Configuration file for connector with name: %s not found!",
                            connector_config_from_main["name"],
                        )
                        continue
                    ...
                    if isinstance(connector_conf_from_file, dict):
                        if connector_conf_from_file.get("id") is None:
                            ...
                            with open(
                                connector_config_file_path, "w", encoding="UTF-8"
                            ) as conf_file:
                    ...
                    connector_config_version = (
                        connector_configuration.get(CONFIG_VERSION_PARAMETER)
                        if isinstance(connector_configuration, dict)
                        else None
                    )
                    connector_configuration_local = {
                        "name": connector_config_from_main["name"],
                        "id": connector_id,
                    ...
                    if (
                        isinstance(connector_conf_from_file, dict)
                        and connector_conf_from_file.get(REPORT_STRATEGY_PARAMETER) is not None
                    ):  # noqa
                        connector_configuration_local[REPORT_STRATEGY_PARAMETER] = (
                            connector_conf_from_file[REPORT_STRATEGY_PARAMETER]
                        )  # noqa
    ...
    def __update_connector_devices(self, connector):
        for device_name in set(self.__connected_devices.keys()):
            ...
            if device.get("connector") and (
                device["connector"].name == connector.name
                or device["connector"].get_id() == connector.get_id()
            ):
    ...
    def get_connector_devices(self, connector):
        return [
            ...
            if device.get("connector") and device["connector"].get_id() == connector.get_id()
        ]  # noqa

    def __cleanup_connectors(self):
        self.available_connectors_by_id = {
            connector_id: connector
            for (connector_id, connector) in self.available_connectors_by_id.items()
            if not connector.is_stopped()
        }
        ...
        for connector_type in self.connectors_configs:
            ...
            for connector_config in self.connectors_configs[connector_type]:
                if self._implemented_connectors.get(connector_type) is not None:
                    ...
                    if (
                        connector_type == "grpc"
                        or "Grpc" in self._implemented_connectors[connector_type].__name__
                    ):
                    ...
                    for config_file_name in connector_config[CONFIG_SECTION_PARAMETER]:
                        ...
                        connector_name = None
                        connector_id = None
                        connector_configuration = connector_config[CONFIG_SECTION_PARAMETER].get(
                            config_file_name
                        )
                        try:
                            connector_name = connector_config["name"]
                            connector_id = connector_config["id"]
                            connector = self.__init_and_start_regular_connector(
                                connector_id,
                                connector_type,
                                connector_name,
                                connector_configuration,
                            ...
                            log.error(
                                "[%r] Error on loading connector %r: %s",
                                connector_id,
                                connector_name,
                                e,
                            )
                            if isinstance(log, TbLogger):
                                log.error(
                                    "Error on loading connector %r: %s",
                                    connector_name,
                                    e,
                                    attr_name=connector_name,
                                )
                            else:
                                log.error("Error on loading connector %r: %s", connector_name, e)
                                log.debug(
                                    "Error on loading connector %r", connector_name, exc_info=e
                                )
    ...
    def __init_and_start_regular_connector(self, _id, _type, name, configuration):
        ...
        if configuration is not None and self.__check_connector_configuration(configuration):
            ...
            if available_connector is None or available_connector.is_stopped():
                connector = self._implemented_connectors[_type](
                    self, deepcopy(configuration), _type
                )
                connector.name = name
                self.available_connectors_by_id[_id] = connector
                self.available_connectors_by_name[name] = connector
                try:
                    report_strategy_config_connector = configuration.pop(
                        REPORT_STRATEGY_PARAMETER, None
                    )  # noqa
                    connector_report_strategy = ReportStrategyConfig(
                        report_strategy_config_connector
                    )  # noqa
                    if self._report_strategy_service is not None:
                        self._report_strategy_service.register_connector_report_strategy(
                            name, _id, connector_report_strategy
                        )  # noqa
                except ValueError:
                    log.info(
                        "Cannot find separated report strategy for connector %r. \
                ...
                log.debug(
                    "[%r] Connector with name %s already exists and not stopped, skipping updating it...",
                    _id,
                    name,
                )
                try:
    ...
    def __init_and_start_grpc_connector(self, _type, configuration):
        ...
        if _type != "grpc":
            ...
            connector_abs_path = (
                f"{connector_dir_abs}/grpc_connectors/{_type}/{connector_file_name}"  # noqa
            )
            connector_config_json = dumps(
                {
                    **configuration,
                    "gateway": {"host": "localhost", "port": self.__config["grpc"]["serverPort"]},
                }
            )
                ...
                for connector_config in self.connectors_configs[connector_type]:
                    for index, connector in enumerate(self.__config["connectors"]):
                        if connector_config["config"].get(connector["configuration"]):
                            self.__config["connectors"][index]["configurationJson"] = (
                                connector_config["config"][connector["configuration"]]
                            )
    ...
    def send_to_storage(
        self, connector_name, connector_id, data: Union[dict, ConvertedData] = None
    ):
        if data is None:
            ...
            if isinstance(data, ConvertedData):
                if data.metadata and self.__latency_debug_mode:
                    data.add_to_metadata(
                        {
                            SEND_TO_STORAGE_TS_PARAMETER: int(time() * 1000),
                            CONNECTOR_PARAMETER: connector_name,
                        }
                    )
            filtration_start = time() * 1000
            if self._report_strategy_service is not None:
                self._report_strategy_service.filter_data_and_send(
                    data, connector_name, connector_id
                )
            else:
    ...
    def __process_event(self, task):
        ...
        if converted_data_format:
            if self.__latency_debug_mode:
                event.add_to_metadata(
                    {"getFromConvertedDataQueueTs": int(time() * 1000), "connector": connector_name}
                )
            ...
            if self.__latency_debug_mode and event.metadata.get(DATA_RETRIEVING_STARTED):
                log.debug(
                    "Data retrieving and conversion took %r ms",
    ...
    def __send_to_storage_new_formatted_data(
        self, connector_name, connector_id, data_array: List[ConvertedData]
    ):
        ...
        for data in data_array:
            ...
            if connector_name == self.name:
                ...
                if not TBUtility.validate_converted_data(data):
                    log.error(
                        "[%r] Data from %s connector is invalid.", connector_id, connector_name
                    )
                    continue
                ...
                if self.tb_client.is_connected() and (
                    data.device_name not in self.get_devices()
                    or data.device_name not in self.__connected_devices
                ):
                    if self.available_connectors_by_id.get(connector_id) is not None:
                        ...
                        self.add_device(
                            data.device_name,
                            {
                                CONNECTOR_PARAMETER: self.available_connectors_by_name[
                                    connector_name
                                ]
                            },
                            device_type=data.device_type,
                        ...
                        log.trace(
                            "Connector %s is not available, probably it was disabled, skipping data...",
                            connector_name,
                        )
                        continue
                ...
                adopted_data_max_entry_size = (
                    max_data_size - DEBUG_METADATA_TEMPLATE_SIZE - len(connector_name)
                    if self.__latency_debug_mode
                    else max_data_size
                )
                start_splitting = int(time() * 1000)
                adopted_data: List[ConvertedData] = data.convert_to_objects_with_maximal_size(
                    adopted_data_max_entry_size
                )  # noqa
                end_splitting = int(time() * 1000)
                if self.__latency_debug_mode:
                    log.trace(
                        "Data splitting took %r ms, telemetry datapoints count: %r, attributes count: %r",
        ...
        for data in data_array:
            if not connector_name == self.name:
                ...
                if self.tb_client.is_connected() and (
                    ...
                    or data["deviceName"] not in self.__connected_devices
                ):
                    if self.available_connectors_by_id.get(connector_id) is not None:
                        self.add_device(
                            data["deviceName"],
                ...
                for ts_kv_list in telemetry:
                    ...
                    for kv in ts_kv_list["values"]:
                        ...
                        if adopted_data_size >= max_data_size:
                            ...
                            self.__send_data_pack_to_storage(
                                adopted_data, connector_name, connector_id
                            )
        ...
        for item in data["telemetry"]:
            if item.get("ts") is None:
                ...
                if isinstance(item["ts"], int):
                    ...
                    log.warning(
                        "Data has invalid TS (timestamp) format! Using generated TS instead."
                    )
    ...
    def __send_data_pack_to_storage(self, data, connector_name, connector_id=None):
        if isinstance(data, ConvertedData):
            ...
            json_data = dumps(
                data.to_dict(self.__latency_debug_mode),
                separators=(",", ":"),
                skipkeys=True,
                ignore_nan=True,
            )
        ...
        if not save_result:
            log.error(
                '%rData from the device "%s" cannot be saved, connector name is %s.',
                "[" + connector_id + "] " if connector_id is not None else "",
                data.device_name if isinstance(data, ConvertedData) else data["deviceName"],
                connector_name,
            )
    ...
    def __read_data_from_storage(self):
        ...
        log.debug(
            "Maximal size of the client message queue is: %r",
                    ...
                    if events:
                        ...
                        for event in events:
                            try:
                                ...
                                log.error(
                                    "Error while processing event from the storage, it will be skipped.",
                                    exc_info=e,
                                )
                                continue
                            ...
                            if current_event.get("telemetry"):
                                if isinstance(current_event["telemetry"], list):
                                    ...
                                    if has_metadata and current_event["telemetry"].get("ts"):
                                        current_event["telemetry"].update(
                                    ...
                                    telemetry_dp_count += len(
                                        current_event["telemetry"].get("values", [])
                                    )
                        ...
                        log.debug(
                            "Telemetry dp count: %r and attributes dp count: %r. Counting took: %r milliseconds.",  # noqa
                            telemetry_dp_count,
                            attribute_dp_count,
                            int((time() - start_pack_processing) * 1000),
                        )  # noqa
                        if devices_data_in_event_pack:
                            ...
                            if self.__latency_debug_mode and events_len > 100:
                                ...
                                if average_event_processing_time < 1.0:
                                    ...
                                    average_event_processing_time_str = (
                                        f"{average_event_processing_time:.2f} milliseconds."  # noqa
                                    )
                                log.debug(
                                    "Sending data to ThingsBoard, pack size %i processing took %i ,milliseconds. Average event processing time is %s",  # noqa
                                    events_len,
                        ...
                        if self.tb_client.is_connected() and (
                            ...
                            if success and self.tb_client.is_connected():
                                ...
                                devices_data_in_event_pack = {}
                                StatisticsService.add_count(
                                    "platformTsProduced", count=telemetry_dp_count
                                )
                                StatisticsService.add_count(
                                    "platformAttrProduced", count=attribute_dp_count
                                )
    ...
    def __handle_published_events(self):
        ...
        try:
            if self.tb_client.is_connected() and (
                ...
                if qos == 1:
                    futures = list(
                        self.__messages_confirmation_executor.map(
                            self.__process_published_event, events
                        )
                    )
            ...
            for device in devices_data_in_event_pack:
                ...
                if devices_data_in_event_pack[device].get("attributes"):
                    if device == self.name or device == "currentThingsBoardGateway":
                        ...
                        self._published_events.put(
                            self.gw_send_attributes(
                                final_device_name, devices_data_in_event_pack[device]["attributes"]
                            )
                        )
                if devices_data_in_event_pack[device].get("telemetry"):
                    if device == self.name or device == "currentThingsBoardGateway":
                        ...
                        self._published_events.put(
                            self.gw_send_telemetry(
                                final_device_name, devices_data_in_event_pack[device]["telemetry"]
                            )
                        )
                        ...
                        if self.connectors_configs.get(module):
                            ...
                            for connector_name in self.available_connectors_by_name:
                                if (
                                    ...
                                    result = self.available_connectors_by_name[
                                        connector_name
                                    ].server_side_rpc_handler(
                                        content
                                    )  # noqa E501
                        elif module == "gateway" or (
                            self.__remote_shell and module in self.__remote_shell.shell_commands
                        ):  # noqa
        ...
        while not self.stopped:
            try:
                ...
                if monotonic() - received_time > timeout:
                    ...
                    self.send_rpc_reply(
                        content["device"], request_id, '{"error":"Request timeout", "code": 408}'
                    )
                    continue
                device = content.get("device")
                ...
                if device in self.get_devices():
                    ...
                    if connector is not None:
                        ...
                        if result is not None and isinstance(result, dict) and "error" in result:
                            self.send_rpc_reply(
                                device, request_id, dumps(result), success_sent=False
                            )
                    else:
                        log.error(
    ...
    def __rpc_gateway_processing(self, request_id, content):
        ...
        if method_function is None and method_to_call in self.__rpc_scheduled_methods_functions:
            ...
            log.info(
                "Gateway %s scheduled in %i seconds", method_to_call, seconds_to_restart / 1000
            )
            result = {"success": True}
        ...
        try:
            ...
            if device is not None and success_sent is not None and not to_connector_rpc:
                self.tb_client.client.gw_send_rpc_reply(
                    device, req_id, dumps(rpc_response), quality_of_service=quality_of_service
                )
            elif (
                device is not None
                and req_id is not None
                and content is not None
                and not to_connector_rpc
            ):
                self.tb_client.client.gw_send_rpc_reply(
    ...
    def _attribute_update_callback(self, content, *args):
        ...
        if device_name is not None and (
            "value" in content or "values" in content or "data" in content
        ):
            ...
            try:
                target_device_name = TBUtility.get_dict_key_by_value(
                    self.__renamed_devices, device_name
                )
                ...
                if self.__sync_devices_shared_attributes_on_connect:
                    if target_device_name in self.__devices_shared_attributes:
                        self.__devices_shared_attributes[target_device_name].update(
                            content["data"]
                        )  # noqa
                    else:
                ...
                if self.__connected_devices.get(target_device_name) is not None:
                    device_connector = self.__connected_devices[target_device_name][
                        CONNECTOR_PARAMETER
                    ]
    ...
    def __form_statistics(self):
        ...
        for connector in self.available_connectors_by_name:
            ...
            telemetry = {
                (connector_camel_case + " EventsProduced")
                .replace(" ", ""): self.available_connectors_by_name[connector]
                .statistics.get("MessagesReceived", 0),  # noqa
                (connector_camel_case + " EventsSent")
                .replace(" ", ""): self.available_connectors_by_name[connector]
                .statistics.get("MessagesSent", 0),  # noqa
            }
    ...
    def add_device(self, device_name, content, device_type=None):
        ...
        if device_name in self.__renamed_devices:
            if self.__sync_devices_shared_attributes_on_connect and hasattr(
                content["connector"], "get_device_shared_attributes_keys"
            ):
                self.__sync_device_shared_attrs_queue.put(
                    (self.__renamed_devices[device_name], content["connector"])
                )
        ...
        if (
            device_name in self.__connected_devices
            or TBUtility.get_dict_key_by_value(self.__renamed_devices, device_name) is not None
        ):
            if self.__sync_devices_shared_attributes_on_connect and hasattr(
                content["connector"], "get_device_shared_attributes_keys"
            ):
        ...
        if device_name in self.__saved_devices:
            if content.get(CONNECTOR_PARAMETER) is not None:
                ...
                try:
                    if self.__added_devices.get(device_name) is None or (
                        self.__added_devices[device_name]["device_details"]["connectorType"]
                        != connector_type
                        or self.__added_devices[device_name]["device_details"]["connectorName"]
                        != connector_name
                    ):
                        device_details = {
                        ...
                        self.__added_devices[device_name] = {
                    ...
                    global log
                    log.error(
                        "Error on sending device details about the device %s",
                        device_name,
                        exc_info=e,
                    )
                    return False
    ...
    def __process_sync_device_shared_attrs(self, device_name, connector):
        ...
        if device_name in self.__devices_shared_attributes:
            ...
            shared_attributes_request = {"device": device_name, "data": device_shared_attrs}
    ...
    def update_device(self, device_name, event, content: Connector):
        ...
        if self.__connected_devices.get(device_name) is None:
            return
        if event == "connector" and (
            self.__connected_devices[device_name].get(event) != content
        ...
        if should_save:
            ...
            info_to_send = {
                DatapointKey(
                    "connectorName", ReportStrategyConfig({"type": ReportStrategy.ON_RECEIVED.name})
                ): content.get_name()
            }
            if device_name in self.__connected_devices:  # TODO: check for possible race condition
                self.send_to_storage(
                    connector_name=content.get_name(),
    ...
    def __load_persistent_connector_keys(self):
        ...
        if (
            ...
            and path.getsize(self._config_dir + PERSISTENT_GRPC_CONNECTORS_KEY_FILENAME) > 0
        ):
            try:
                ...
                log.error(
                    "Error while loading persistent keys from file with error: %s", e, exc_info=e
                )
                ...
                try:
                    ...
                    if isinstance(
                        ...
                        if connector is None:
                            log.warning(
                                "Connector with id %s not found, trying to use connector by name!",
                                device_connector_id,
                            )
                        ...
                        if connector is None:
                            log.debug(
                                "Connector with name %s not found! probably it is disabled, device %s will be "
                    ...
                    for device in list(self.__connected_devices.keys()):
                        if device in self.__connected_devices:
                            self.add_device(
                                device,
                                self.__connected_devices[device],
                                self.__connected_devices[device][DEVICE_TYPE_PARAMETER],
                            )
                    self.__saved_devices[device_name] = device_data_to_save

                except Exception as e:
                    log.error(
                        "Error while loading connected devices from file with error: %s",
                        e,
                        exc_info=e,
                    )
                    continue
        ...
        with self.__lock:
            ...
            with open(self._config_dir + CONNECTED_DEVICES_FILENAME, "w") as config_file:
                try:
                    ...
                    log.error(
                        "Error while saving connected devices to file with error: %s", e, exc_info=e
                    )
    ...
    def send_telemetry(self, telemetry, wait_for_publish=True):
        return self.tb_client.client.send_telemetry(
            telemetry, quality_of_service=self.quality_of_service, wait_for_publish=wait_for_publish
        )

    @CountMessage("msgsSentToPlatform")
    def gw_send_telemetry(self, device, telemetry):
        return self.tb_client.client.gw_send_telemetry(
            device, telemetry, quality_of_service=self.quality_of_service
        )

    @CountMessage("msgsSentToPlatform")
    def send_attributes(self, attributes, wait_for_publish=True):
        return self.tb_client.client.send_attributes(
            attributes,
    ...
    def gw_send_attributes(self, device, attributes):
        return self.tb_client.client.gw_send_attributes(
            device, attributes, quality_of_service=self.quality_of_service
        )
